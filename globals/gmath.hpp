...
inline int dmgCalc(int atk, int def){
    if(atk >= def){
        return atk-(def/2);
    }
    else{
...
inline sf::Vector2f lerp(sf::Vector2f a, sf::Vector2f b, float f){
...
inline sf::Vector2f calculateJumpArc(sf::Vector2f start, sf::Vector2f end, float height, float t){
